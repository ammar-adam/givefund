...
_CARD_SELECTORS = [
    ...
    "[class*='campaign-card']",
    "[class*='ProjectCard']",
    "article a[href*='launchgood']",
]

_TITLE_SELECTORS = ["h2", "h3", "[class*='title']"]
...
async def _extract_campaign(card: ElementHandle) -> Optional[dict]:
    """Extract one LaunchGood campaign from a card or link element."""
    try:
        ...
        campaign_url = href if href.startswith("http") else f"https://www.launchgood.com{href}"
        ...
        logger.error("LaunchGood card extraction failed: %s", exc)
        return None


async def scrape_launchgood() -> list[dict]:
    """Scrape LaunchGood discover with pagination."""
    ...
    async with async_playwright() as pw:
        ...
        try:
            for page_num in range(1, PAGES_PER_CATEGORY + 1):
                url = f"{DISCOVER_URL}?page={page_num}"
                logger.info("[launchgood] page %d -- %s", page_num, url)
                try:
                    await page.goto(url, wait_until="domcontentloaded", timeout=30_000)
                    await page.wait_for_timeout(2000)
                except Exception as exc:
                    logger.error("[launchgood] page %d failed: %s", page_num, exc)
                    break

                cards = await find_cards(page, _CARD_SELECTORS)
                if not cards:
                    logger.info("[launchgood] page %d: empty, stopping", page_num)
                    break

                logger.info("[launchgood] page %d: %d cards", page_num, len(cards))
                seen: set[str] = set()
                for card in cards:
